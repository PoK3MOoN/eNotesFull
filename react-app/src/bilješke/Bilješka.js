    ...
    if (odgovor) {
      const url = await fetch(
        "https://e-notes-4mhk.onrender.com/obrisiBiljesku",
        {
          ...
          body: await JSON.stringify({ id: idBilješke }),
        }
      );
      navigate("/");
    }
  ...
  const setNaslovCB = (e) => {
    ...
    if (e.target)
      e.target.value.length <= 18
        ...
        : alert("Naslov ne može sadržavati više od 18 znakova");
    ...
    <div>
      <div className="bg-transparent w-screen h-screen centriraj">
        <span className="grid w-[90vw]  lg:w-[1000px] md:-translate-y-24">
          ...
          {!učitavanje && (
            <span>
              <div className="w-full flex flex-col gap-4 rounded-3xl">
                {/* NASLOV */}
                <div
                  className="px-4 flex flex-col items-left sm:flex-row sm:items-center gap-3 sm:gap-4 h-20 w-full hover:cursor-pointer group"
                  onClick={setPromjenaNaslova.bind(true)}
                >
                  {!promjenaNaslova && (
                    <h1 className="text-lg h-6 sm:text-2xl md:text-4xl font-semibold text-white ">
                      {naslov}
                    </h1>
                  )}
                  {promjenaNaslova && (
                    <input
                      className="w-[80%] overflow-hidden h-6 text-lg sm:text-2xl md:text-4xl text-white font-semibold focus:border-0 bg-transparent border-none outline-none"
                  ...
                  <div className="h-10 sm:mb-0 sm:ml-auto flex gap-4 ">
                    <svg
                      ...
                      stroke-width="1.5"
                      stroke="white"
                      class="w-10 h-10 md:w-12 md:h-12  flex align-middle justify-center group-hover:rotate-6 animacija mb-4"
                    >
                      <path
